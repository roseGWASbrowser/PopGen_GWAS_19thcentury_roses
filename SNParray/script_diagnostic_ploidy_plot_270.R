...
##########################################################
##########################################################
dir.create("ploidy_diagnostic_plots")
...
for (i in list_geno) {
  ...
  tiff(paste("ploidy_diagnostic_plots/",i,".tiff",sep=""),width = 2500, height = 2000,res=180, type="cairo")
...
# passing markers in bp genome 
  temp = subset(df_score_order_OB,df_score_order_OB$sample == i & df_score_order_OB$scored_probes =="yes")  
  dim(temp) 
  
  cols <- temp$maxgeno
  cols=as.factor(as.character(cols))
  levels(cols)[levels(cols)=="0"] <- "black"
  levels(cols)[levels(cols)=="1"] <- "darkred"
  levels(cols)[levels(cols)=="2"] <- "green"
  levels(cols)[levels(cols)=="3"] <- "navyblue"
  levels(cols)[levels(cols)=="4"] <- "cyan"
  
  plot(temp$Position_OB_genome_bp_cumulative/10^6,temp$mean_score,pch=19,
       ylab="Weighted average score (allelic dose)",
       xlab="Position on the Old Blush genome",xaxt='n',
       main=i, col=alpha(cols, 0.4) , cex=0.5)
  
  mtext(side=3, cex=1, paste("expected ploidy :",unique(temp$ploidy)),col="black")
  
  axis(side = 1, at = c(  (max(temp[which(temp$Chromosome_OB == "Chr01"),"Position_OB_genome_bp_cumulative"])-min(temp[which(temp$Chromosome_OB == "Chr01"),"Position_OB_genome_bp_cumulative"]))/2 + min(temp[which(temp$Chromosome_OB == "Chr01"),"Position_OB_genome_bp_cumulative"]),
                          (max(temp[which(temp$Chromosome_OB == "Chr02"),"Position_OB_genome_bp_cumulative"])-min(temp[which(temp$Chromosome_OB == "Chr02"),"Position_OB_genome_bp_cumulative"]))/2 + min(temp[which(temp$Chromosome_OB == "Chr02"),"Position_OB_genome_bp_cumulative"]),
                          (max(temp[which(temp$Chromosome_OB == "Chr03"),"Position_OB_genome_bp_cumulative"])-min(temp[which(temp$Chromosome_OB == "Chr03"),"Position_OB_genome_bp_cumulative"]))/2 + min(temp[which(temp$Chromosome_OB == "Chr03"),"Position_OB_genome_bp_cumulative"]),
                          (max(temp[which(temp$Chromosome_OB == "Chr04"),"Position_OB_genome_bp_cumulative"])-min(temp[which(temp$Chromosome_OB == "Chr04"),"Position_OB_genome_bp_cumulative"]))/2 + min(temp[which(temp$Chromosome_OB == "Chr04"),"Position_OB_genome_bp_cumulative"]),
                          (max(temp[which(temp$Chromosome_OB == "Chr05"),"Position_OB_genome_bp_cumulative"])-min(temp[which(temp$Chromosome_OB == "Chr05"),"Position_OB_genome_bp_cumulative"]))/2 + min(temp[which(temp$Chromosome_OB == "Chr05"),"Position_OB_genome_bp_cumulative"]),
                          (max(temp[which(temp$Chromosome_OB == "Chr06"),"Position_OB_genome_bp_cumulative"])-min(temp[which(temp$Chromosome_OB == "Chr06"),"Position_OB_genome_bp_cumulative"]))/2 + min(temp[which(temp$Chromosome_OB == "Chr06"),"Position_OB_genome_bp_cumulative"]),
                          (max(temp[which(temp$Chromosome_OB == "Chr07"),"Position_OB_genome_bp_cumulative"])-min(temp[which(temp$Chromosome_OB == "Chr07"),"Position_OB_genome_bp_cumulative"]))/2 + min(temp[which(temp$Chromosome_OB == "Chr07"),"Position_OB_genome_bp_cumulative"]),
                          (max(temp[which(temp$Chromosome_OB == "Chr00"),"Position_OB_genome_bp_cumulative"])-min(temp[which(temp$Chromosome_OB == "Chr00"),"Position_OB_genome_bp_cumulative"]))/2 + min(temp[which(temp$Chromosome_OB == "Chr00"),"Position_OB_genome_bp_cumulative"]))/10^6, 
              labels = c("Ch01","Ch02","Ch03","Ch04","Ch05","Ch06","Ch07","Ch00"))
  
  
  abline(v=max(temp[which(temp$Chromosome_OB == "Chr01"),"Position_OB_genome_bp_cumulative"])/10^6,lty=2)
  abline(v=max(temp[which(temp$Chromosome_OB == "Chr02"),"Position_OB_genome_bp_cumulative"])/10^6,lty=2)
  abline(v=max(temp[which(temp$Chromosome_OB == "Chr03"),"Position_OB_genome_bp_cumulative"])/10^6,lty=2)
  abline(v=max(temp[which(temp$Chromosome_OB == "Chr04"),"Position_OB_genome_bp_cumulative"])/10^6,lty=2)
  abline(v=max(temp[which(temp$Chromosome_OB == "Chr05"),"Position_OB_genome_bp_cumulative"])/10^6,lty=2)
  abline(v=max(temp[which(temp$Chromosome_OB == "Chr06"),"Position_OB_genome_bp_cumulative"])/10^6,lty=2)
  abline(v=max(temp[which(temp$Chromosome_OB == "Chr07"),"Position_OB_genome_bp_cumulative"])/10^6,lty=2)
...
# passing markers in bp genome 
  ...
  plot(temp$Position_OB_genome_bp_cumulative/10^6,temp$ratio,pch=19,
       ylab="Ratio",
       xlab="Position on the Old Blush genome",xaxt='n',
       main=i, col=alpha(cols, 0.4) , cex=0.5)
...
temp = subset(df_score,df_score$sample == i & df_score$scored_probes =="yes")  
dim(temp) 
 
 
hist(temp$mean_score,col="lightblue",xlim=c(0,4),breaks=seq(0,4,0.01),
       xlab="Weighted average score (allelic dose)",main="",ylab="Probe counts")
...
hist(temp$ratio,col="lightblue",xlim=c(0,1),breaks=seq(0,1,0.01),
     xlab="Ratio",main="",ylab="Probe counts")
mtext(side=3, cex=1, paste("expected ploidy :",unique(temp$ploidy)),col="black")
...
lines(density(temp$ratio,na.rm=TRUE),col="blue", lwd=2.5,bty='l',main="")
